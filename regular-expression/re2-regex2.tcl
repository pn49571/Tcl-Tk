...
proc t1 {} {
    thread::create {
        ...
        sqlite3 db1 /home/sai/database/10-shards/syn_data1
        proc regu {arg} {
            set a [regular $arg s.*d]
            return $a
        }
        db1 function regexp regu
        puts "[db1 eval {SELECT *  FROM contracts WHERE regexp(first_name)='t'}]"
        set end [clock microseconds]
        puts "1thread: [expr {$end -$start}]"
    }
}
...
proc t1 {} {
    thread::create {
        ...
        db1 function regexp -deterministic {regu} 
        puts "[db1 eval {SELECT *  FROM contracts WHERE regexp(first_name)='t'}]"
        set end [clock microseconds]
        puts "1thread: [expr {$end -$start}]"
    }
...
###############################################################################################
##Re2 operation - combining thread operations
###############################################################################################
package require Thread
tsv::set myarray sum " "
tsv::set counter sumo 0
...
proc t1 {} {
    thread::create {
        ...
        db1 function regexp regu
        set res "[db1 eval {SELECT *  FROM contracts WHERE regexp(first_name)='t'}]"
        tsv::append myarray sum $res
        tsv::incr counter sumo 1
        # puts "first counter: [tsv::get counter sumo]"
        # set end [clock microseconds]
